...
pub struct DefaultNodeIter<'a>(std::slice::Iter<'a, Node>);

impl<'a> Iterator for DefaultNodeIter<'a> {
    type Item = Node;
...
impl<'a> Nodes<'a> for Vec<Node> {
    type NodeIter = DefaultNodeIter<'a>;

    fn nodes(&'a self) -> Self::NodeIter {
        DefaultNodeIter(self.iter())
    }
}
